...
@JvmInline
value class NonConsecutiveClosedRange<T : Comparable<T>> internal constructor(private val ranges: Iterable<ClosedRange<T>>) : ClosedRange<T> {
	override val start: T
		get() = ranges.maxOf { it.endInclusive }
	override val endInclusive: T
		get() = ranges.minOf { it.start }

	override fun isEmpty(): Boolean = !ranges.any()

	override fun contains(value: T): Boolean = ranges.any { value in it }
