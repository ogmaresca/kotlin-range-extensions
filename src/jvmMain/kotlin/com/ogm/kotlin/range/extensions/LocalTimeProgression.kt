...
class LocalTimeProgression private constructor(
	start: LocalTime,
	...
	companion object {
		fun fromClosedRange(rangeStart: LocalTime, rangeEnd: LocalTime, step: Duration) =
			LocalTimeProgression(rangeStart, rangeEnd, step)

		fun fromClosedRange(rangeStart: LocalTime, rangeEnd: LocalTime, stepSize: Long, stepUnit: TemporalUnit) =
			fromClosedRange(rangeStart, rangeEnd, Duration.of(stepSize, stepUnit))

		fun fromClosedRange(rangeStart: LocalTime, rangeEnd: LocalTime, stepUnit: TemporalUnit) =
			fromClosedRange(rangeStart, rangeEnd, 1L, stepUnit)

		fun fromClosedRange(range: ClosedRange<LocalTime>, step: Duration) =
			fromClosedRange(range.start, range.endInclusive, step)

		fun fromClosedRange(range: ClosedRange<LocalTime>, stepSize: Long, stepUnit: TemporalUnit) =
			fromClosedRange(range.start, range.endInclusive, stepSize, stepUnit)

		fun fromClosedRange(range: ClosedRange<LocalTime>, stepUnit: TemporalUnit) =
